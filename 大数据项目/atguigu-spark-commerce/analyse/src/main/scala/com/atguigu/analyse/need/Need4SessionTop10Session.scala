 ...
 * @Description: Top10 热门品类 Top10 活跃 Session 统计
**/
object Need4SessionTop10Session {
  ...
  def main(args: Array[String]): Unit = {
    ...
    getTop10Session(sparkSession, taskUUID, top10CategoryList, sessionid2detailRDD)
  ...
  /**
    ...
    * @Description: 获取top10热门品类的活跃session
    *               第一步: 找出这top10品类的categoryid
    *               第二步: 每一个品类被每一个session点击的次数
    *               第三步: 对第二步结果以次数排序,取top10的session
    *               第四步: 写入sql
    *               第五步: 获取top10session的明细数据.
    *
    **/
  def getTop10Session(sparkSession: SparkSession, taskUUID: String, top10CategoryList: Array[(CategorySortKey, String)], sessionid2detailRDD: RDD[(String, UserVisitAction)]) = {
    // 第一步：将top10热门品类的id，生成一份RDD
    val categoryIds: Array[(Long, Long)] = top10CategoryList.map { case (key, line) =>
      val categoryid = StringUtils.getFieldFromConcatString(line, "\\|", Constants.FIELD_CATEGORY_ID).toLong;
      (categoryid, categoryid)
    }
    val categoryIdRDD: RDD[(Long, Long)] = sparkSession.sparkContext.makeRDD(categoryIds)

    // 第二步：计算top10品类被各session点击的次数
    val sessionid2ActionsRDD: RDD[(String, Iterable[UserVisitAction])] = sessionid2detailRDD.groupByKey()

    // 获取每个品类被每一个Session点击的次数
    //@return  (categoryid,session+count])
    //TODO 为啥用flatmap?,为啥用yield?
    val category2SessionCountRDD: RDD[(Long, String)] = sessionid2ActionsRDD.flatMap { case (sessionId, userVisitActionList) =>

      val clickCountMap: mutable.HashMap[Long, Long] = new mutable.HashMap[Long, Long]
      for (userVisitAction <- userVisitActionList) {
        val clickCategoryId: Long = userVisitAction.click_category_id

        if (!clickCountMap.contains(clickCategoryId)) {
          //不存在就初始化为1
          clickCountMap.put(clickCategoryId, 1L)
        } else {
          //存在,就+1
          clickCountMap.put(clickCategoryId, clickCountMap(clickCategoryId) + 1)
        }
      }
      // 对categoryCountMap中的数据进行格式转化
      //TODO  这一步怎么解释?
      for ((categoryid, count) <- clickCountMap)
    ...
    //@return  (categoryid,(categoryid,session+count))
    val top10category2SessionCountRDD: RDD[(Long, (Long, String))] = categoryIdRDD.join(category2SessionCountRDD)

    //分组
    //@return (category,List(category,session+count))
    val top10category2SessionCountListRDD: RDD[(Long, Iterable[(Long, String)])] = top10category2SessionCountRDD.groupByKey()

    // 将每一个品类的所有点击排序，取前十个，并转换为对象
    val top10SessionObjectRDD: RDD[Top10Session] = top10category2SessionCountListRDD.flatMap { case (category, clicks) =>
      val top10Sessions: List[Top10Session] = clicks.toList.map {
      ...
      //按照 clickCount 排序
      val top10SessionsSort: List[Top10Session] = top10Sessions.sortBy(bean => bean.clickCount)
      val top10: List[Top10Session] = top10SessionsSort.take(10)
      top10
    }
    ...
    val top10SessionRDD = top10SessionObjectRDD.map(item => (item.sessionid, 1))

    // 第四步：获取top10活跃session的明细数据
    val sessionDetailRDD = top10SessionRDD.join(sessionid2detailRDD).map { case (sid, (sessionid, userVisitAction)) =>
