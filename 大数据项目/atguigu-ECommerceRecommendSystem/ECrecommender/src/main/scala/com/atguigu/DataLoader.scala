...
/**
  ...
  * 1395676800  时间戳
  */
case class MyRating( userId: Int, productId: Int, score: Double, timestamp: Int )
...
object DataLoader {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val ratingDF:DataFrame = ratingRDD.map( item => {
      ...
      MyRating( attr(0).toInt, attr(1).toInt, attr(2).toDouble, attr(3).toInt )
    } ).toDF()
