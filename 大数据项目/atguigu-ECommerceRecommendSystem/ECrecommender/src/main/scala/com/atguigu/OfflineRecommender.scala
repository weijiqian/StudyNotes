...
object OfflineRecommender {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // 提取出所有用户和商品的数据集
    val userRDD:RDD[Int] = ratingRDD.map(rating => rating._1).distinct()
    val productRDD:RDD[Int] = ratingRDD.map(rating => rating._2).distinct()

    // 核心计算过程
    // 1. 训练隐语义模型
    //Rating(x._1,x._2,x._3) 三个参数依次为:用户,评价的东西,评分.
    val trainData:RDD[Rating] = ratingRDD.map(x=>Rating(x._1,x._2,x._3))
    // 定义模型训练的参数，rank隐特征个数，iterations迭代词数，lambda正则化系数
    //这三个参数是通过ALSTrainer 训练得到的.
